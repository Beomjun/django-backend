...
class TestUserQueries(CustomGraphQLTestCase):
    ...
    def test_user_query(self):
        response = self.query_with_auth(self.user_query, self.user, self.token, variables={'email': 'no_user@test.com'})
        ...
        response = self.query_with_auth(self.user_query, self.user, self.token, variables={'email': self.user.email})
        content = json.loads(response.content)
        self.assertIn('data', content)
